...
template<class T>
void printTree(Node<T>* root_, int space) {
  ...
  std::cout<< root_->value << color<<std::endl;
  printTree(root_->left, space);
}
...
int main (){
	...
	int option, n;
	while (true) {
		std::cout << " |************* Arbol Red & Black *************| " << std::endl;
