...
// #include <utility>
#include <bits/stdc++.h> // para evitar la fatiga
...
namespace Concurrent {
    ...
    class BLinkNode {
      public:
        ...
        // para test
        data_type* keys_;                  // [key1,key2,key3]  => [B]
        BLinkNode** childs_ptr_;           // Punteros a los hijos => [B + 1]
        bool is_leaf_;                     // Para determinar si es una hojo o nodo interno  # checar esta funcionalidad mas adelante o usamos un flag :x
        std::size_t size_arr_data_;        // Cantidad del arreglo de keys, para determinar si entramos en overflow o no
    ...
    template <std::size_t B, typename Type>  // B = B preguntar ams adelante ////  [1,2,5,6] => 4
    class BLinkTree {
    public:
      typedef Type data_type;
      ...
      //std::size_t size() const {}  // cantidad de datos que se ah insertado o eliminado (dejarlo para el ultimo :x)
      ...
      void insert(const data_type& value) {
        if(search(value)) {
          ...
          if(pivot->size_arr_data_ < B) {
            ...
            {
              insertAuxFunction(new_leaf_->keys_[0], parent,new_leaf_);
            }
          }
        }
      ...
      BLinkNode<B, Type>* findParent(BLinkNode<B, Type>* pivot, BLinkNode<B, Type>* child) {
      ...
      void insertAuxFunction(int value,BLinkNode<B, Type>* pivot,BLinkNode<B, Type>* child) {
      if(pivot->size_arr_data_ < B) {
        int i;
        for(i=0;i<pivot->size_arr_data_;++i)
          if(pivot->keys_[i]>value)
        ...
        if(pivot == root_ptr_) {
          ...
          insertAuxFunction(temp[pivot->size_arr_data_], findParent(root_ptr_,pivot),newleaf);
        }
      }	
    }
