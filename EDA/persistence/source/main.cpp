...
int main() {
  ...
  ADE::Persistence::PartialDirectedGraph<int, ADE::Persistence::PartialNode<int>> my_graph(1, 5, 0);
  ADE::Persistence::PartialNode<int>* root_ptr = my_graph.get_root_ptr(0);
  std::cout << root_ptr->get_data(0) << "\n";

  // my_graph.get_root(0).insert_vertex(0, 2, 0);

  std::pair<std::size_t, unsigned int> PAIR1;
 
    PAIR1.first = 0;
    PAIR1.second = 0;

  my_graph.get_root(0).insert_vertex(0, 2);
  std::cout << "Inserted Value: " << my_graph.get_root(0)[PAIR1].get_data(0)
            << "\n";  // 2
  ...
  my_graph.get_root(0)[PAIR1].insert_vertex(1, 4);
  std::cout << "Inserted Value: " << my_graph.get_root(1)[PAIR1].get_data(0)
            << "\n";  // 2
